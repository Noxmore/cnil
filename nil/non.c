...
static void non_write_reflected_recursive(FILE* file, const type_info* type, const void* data, const u32 depth) {
	if (type->kind == type_info_struct) {
		...
		fputs(reflect_enum_name_from_variant_value(type, value), file);
...
void non_print_error(FILE* file, const non_result result, str source) {
	// TODO: Make good
	usize row;
	usize col;
	str_get_row_col(source, result.loc, &row, &col);
	fprintf(file, "%s at %lu:%lu", error_code_string(result.error), row, col);
}

static non_node_kind node_kind_from_token(enum token_type token_type) {
	switch (token_type) {
		case token_brace_left: return non_map;
		case token_brace_right: UNREACHABLE;
		case token_bracket_left: return non_list;
		case token_bracket_right: UNREACHABLE;
...
non_result non_parse(string s, non_tree* tree) {
	...
	for (usize i = 0; i < tokens.len; i++) {
		...
		// Default values.
		nodes[node_last].next_sibling = NON_VACANT;
		nodes[node_last].first_child = NON_VACANT;

		// Add the latest node as a first child of the parent if needed. Otherwise, add it as a sibling.
		if (node_path[path_last]->first_child == NON_VACANT) {
			node_path[path_last]->first_child = node_last;
		} else {
			nodes[last_child[path_last]].next_sibling = node_last;
		}
		...
		node_path[path_last]->child_count++;

		// If this is parented to a map, make key value pairs.
		if (node_path[path_last]->kind == non_map && nodes[node_last-1].first_child == NON_VACANT) {
			nodes[node_last-1].first_child = node_last;
		}
	}
...
static non_result non_read_into_reflected_recursive(const non_tree* tree, const non_node* node, const type_info* type, void* data) {
	for (u32 child_idx = node->first_child; child_idx != NON_VACANT; child_idx = tree->nodes[child_idx].next_sibling) {
		...
		switch (child->kind) {
			case non_string:
				if (type->conversions.from_string && type->conversions.from_string(data, child->str))
					return non_ok;
				break;
			case non_char:
				if (type->conversions.from_integer && type->conversions.from_integer(data, (s64)child->chr))
					return non_ok;
				break;
			case non_number:
				if (type->conversions.from_floating && type->conversions.from_floating(data, child->num))
					return non_ok;
				break;
			default:
		}
...
non_result non_read_into_reflected(FILE* file, const type_info* type, void* data) {
	...
	non_tree tree;
	non_try(non_parse(s, &tree));

	non_try(non_read_into_reflected_recursive(&tree, &tree.nodes[0], type, data));
